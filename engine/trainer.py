...
def do_train_with_center(
    ...
    evaluator = create_supervised_evaluator(model, metrics={'r1_mAP': R1_mAP(num_query, max_rank=50, feat_norm=cfg.TEST.FEAT_NORM)}, device=device)
    # ignite version problem. for ignite >= 0.50.*, save_interval has been canceled which would lead to problem when training
    checkpointer = ModelCheckpoint(output_dir, cfg.MODEL.NAME, n_saved=10, require_empty=False)  # , checkpoint_period, n_saved=10, require_empty=False)
    timer = Timer(average=True)
